...
import { loadUserData, saveUserData, loadUserSettings, saveUserSettings } from './services/user-data.service.js';
import { sendFriendRequest, fetchFriendRequests, listenForFriends, acceptRequest, declineRequest } from './services/friends.service.js';
...
import { playSoundAndVibrate, notifyFriendsIfReachedLimit } from './utils/notify.util.js';
...
const app = Vue.createApp({
  data() {
    return {
      ...
      showMenu: false, showNotifications: false, showSettings: false,
      showAlert: false, alertMessage: '',
      form: { email: '', password: '', phoneNumber: '', displayName: '' },
      friendIdInput: '', chatMessageInput: '',
      userData: { displayName: '', phoneNumber: '', theme: 'light' },
      thcCalc: {
        gender: 'male', age: 48, weight: 120, bodyFat: 30, frequency: 'often',
        amount: 1.0, thcPercentage: 25,
        lastConsumption: new Date(Date.now()-24*60*60*1000).toISOString().slice(0,16),
        result: { value: null, status: null, waitTime: null }
      },
      landingBannerIndex: 0, landingBanners: [
      ...
      dashboardBannerIndex: 0, dashboardBanners: [
  ...
  methods: {
    ...
    setView(view){
      this.currentView = view; this.showMenu = false;
      if (view === 'dashboard') this.$nextTick(() => { if (!this.map) this.initMap(); });
    ...
    cleanupListeners(){
      this.firestoreListeners.forEach(u => u && u()); this.firestoreListeners = [];
    ...
    async doRegister(){
      ...
      await register({ 
        ...
        displayName: this.form.displayName, phoneNumber: pn
      });
    },
    ...
    async logConsumption(){
      ...
      const q = await db.collection('consumptions')
        .where('userId','==',this.user.uid).where('timestamp','>=',start).get();
      ...
      navigator.geolocation.getCurrentPosition(async pos => {
        ...
        await notifyFriendsIfReachedLimit(this.user.uid, this.userData.displayName || this.user.email, this.settings.consumptionThreshold);
        this.refreshStats();
      }, () => {
    ...
    toggleFriendMarkers(){ setMarkerVisibility(this.map, this.friendMarkers, this.showFriendsOnMap); },

    // --- Events ---
    voteEventUp(id){ return voteEvent(id, this.user.uid, 'up'); },
    voteEventDown(id){ return voteEvent(id, this.user.uid, 'down'); },
    ...
    async actionSendFriendRequest(){
      ...
      alert('Freundschaftsanfrage gesendet!'); this.friendIdInput = '';
    },
    ...
    listenForNotifications(){
      const unsub = db.collection('notifications')
        ...
        .onSnapshot(snap => this.notifications = snap.docs.map(d => ({ id: d.id, ...d.data() })));
      this.firestoreListeners.push(unsub);
    },
    async markNotificationAsRead(n){ if (!n.read) await db.collection('notifications').doc(n.id).update({ read: true }); },
    formatTimestamp,
