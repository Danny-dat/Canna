...
import { setActiveChat, startPresenceHeartbeat, stopPresenceHeartbeat } from '../services/presence.service.js';
...
await sendChatMessage({ fromUid: state.user.uid, toUid: state.activeChat.partner.id, text: txt });
state.chatMessageInput = '';
}

function closeChat(){
unsubscribe?.();
state.activeChat = { chatId:null, partner:null, messages:[], unsubscribe:null };
setActiveChat(state.user.uid, null);
stopPresenceHeartbeat();
}

return { openChat, sendMessage, closeChat, unsubscribe: ()=> unsubscribe && unsubscribe() };
}